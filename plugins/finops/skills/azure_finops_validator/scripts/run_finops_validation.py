...
Runs the full pipeline against a live Azure subscription in one command:
  1. check_permissions.py             -> preflight RBAC report (which roles
                                          are held/missing and what gets skipped)
  2. collect_azure_data.py            -> resource_inventory.json
  3. collect_advisor_and_governance.py -> account_scope_findings.json
  4. validate_resources.py            -> findings.json
  5. generate_report.py               -> PDF report
...
Usage:
    python3 run_finops_validation.py \
        ...
        [--skip-metrics] [--skip-cost] [--skip-preflight]
"""
...
def main():
    ...
    parser.add_argument("--skip-preflight", action="store_true", help="Skip the RBAC preflight check (step 1)")
    ...
    preflight_path = os.path.join(args.output_dir, "preflight_permissions.json")

    if not args.skip_preflight:
        preflight_cmd = [
            sys.executable, os.path.join(SCRIPT_DIR, "check_permissions.py"),
            "--subscription-id", args.subscription_id,
            "--json-output", preflight_path,
        ]
        run_step("1/5 Preflight: verifying identity and required RBAC roles", preflight_cmd)
    ...
    run_step("2/5 Collecting live Azure resource inventory", collect_cmd)
    ...
    run_step("3/5 Collecting Advisor / reservation / budget / subscription-scope findings", governance_cmd)
    ...
    run_step("4/5 Evaluating resources against the FinOps rule catalog", validate_cmd)
    ...
    if os.path.exists(preflight_path):
        report_cmd += ["--preflight", preflight_path]
    run_step("5/5 Generating PDF remediation report", report_cmd)
