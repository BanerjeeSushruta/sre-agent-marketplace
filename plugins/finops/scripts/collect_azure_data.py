...
def populate_properties(resource, autoscale_index, schedule_index):
    ...
    if rtype == "microsoft.compute/virtualmachines":
        ...
        sku = resource.get("sku") or {}
        out["pricing_tier"] = sku.get("name") or (props.get("sku") or {}).get("name")
        out["retention_in_days"] = props.get("retentionInDays")
        daily_quota = (props.get("workspaceCapping") or {}).get("dailyQuotaGb")
        # ARM reports -1.0 when no daily cap is set
        out["daily_quota_gb"] = None if daily_quota in (-1, -1.0) else daily_quota
