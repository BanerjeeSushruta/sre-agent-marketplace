...
Management budgets (AZFO-017), the Dev/Test pricing offer check
(AZFO-019), and Azure Reservation / Savings Plan purchase recommendations
(reported under AZFO-003 with per-SKU detail). Idle load balancer/App
Gateway detection (AZFO-008) and FinOps-tooling sprawl (AZFO-024) still
require manual review — see references/finops_rules.json for their
detection logic.
...
def collect_reservation_recommendations(subscription_id, rule):
    """Concrete Reserved Instance purchase recommendations from the Consumption
    API (same engine as the Azure portal's Reservations > Purchase blade).
    Reported as findings under AZFO-003 so the report's Reservation
    Opportunities section can show SKU/term/quantity/savings detail."""
    findings = []
    try:
        recs = run_az([
        ...
        warn(f"Reservation recommendation list failed (needs Reader; API can lag "
             f"~24h behind new usage): {e}")
        return findings

    for rec in recs:
        sku = rec.get("skuName") or rec.get("sku") or "unknown SKU"
        term = rec.get("term", "P1Y").replace("P1Y", "1-year").replace("P3Y", "3-year")
        qty = rec.get("recommendedQuantity")
        savings = rec.get("netSavings")
        lookback = rec.get("lookBackPeriod", "")
        scope_type = rec.get("scope", "Single")
        detail = (f"Reservation purchase recommendation: {qty} x {sku} ({term} term, "
                  f"{scope_type} scope, based on {lookback or 'recent'} usage)")
        if savings is not None:
            detail += f" — estimated net savings {savings:,.2f} {rec.get('currencyCode', 'USD')} over the term"
        findings.append({
            "rule_id": rule["rule_id"],
            "title": rule["title"],
            "severity": rule["severity"],
            ...
            "resource_name": sku,
            "resource_type": rec.get("resourceType") or "Microsoft.Compute (reservation)",
            "resource_group": None,
...
def main():
    ...
    print("Collecting Azure Reservation purchase recommendations (AZFO-003)...")
    findings += collect_reservation_recommendations(args.subscription_id, rules_by_id["AZFO-003"])
