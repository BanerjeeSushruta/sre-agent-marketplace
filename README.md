...
| [azure-governance](plugins/azure-governance) | Governance review for platform / landing zone / subscription scopes: policy, RBAC, tagging, cost, resiliency, monitoring, security baseline — with a scored report and remediation roadmap. Uses the open-source Azure MCP Server (read-only stdio, no hosting). | [microsoft/mcp](https://github.com/microsoft/mcp) |
...
├── azure-governance/
│   ├── plugin.json
│   ├── .mcp.json                        # Azure MCP Server (stdio, read-only)
│   ├── README.md
│   └── skills/azure_governance/SKILL.md
├── mcp-owasp-top10/
│   ├── plugin.json
│   ├── README.md
